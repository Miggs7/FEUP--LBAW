...
@php
    ...
    $sent_feedback = App\Http\Controllers\ReviewController::getAuthorReviews($id);
    $received_feedback = App\Http\Controllers\ReviewController::getReceivedReviews($id);
    $average_score = App\Http\Controllers\ReviewController::averageScore($id);
      ...
      <div class="card mb-4">
        ...
        <div class="card-body text-center">
          ...
          <span class="d-flex justify-content-center align-items-center">
            <div class="text-danger">{{$average_score}}</div>
          ...
          <button type="button" class="btn btn-primary profile" data-bs-toggle="modal" data-bs-target="#form">
            ...
            <button type="submit" class="btn btn-primary profile mx-2" data-bs-toggle="modal" data-bs-target="#deleteModal">
      ...
      <div class="table-responsive">
        ...
        <table class="table">
          ...
          <tbody>
            ...
            <tr>
              @if($received['id_bidder'] == $id)
              <td>{{App\Http\Controllers\UserController::getUserById($received['id_auctioneer'])->username}} </td>
              @else
              <td>{{App\Http\Controllers\UserController::getUserById($received['id_bidder'])->username}} </td>
              @endif
              <td>{{$received['comment']}}</td>
              <td>{{$received['rating']}}</td>
            </tr>
            @endforeach
          </table>
          </div>
          ...
          <div class="table-responsive">
            ...
            <table class="table">
              ...
              <tbody>
                ...
                <tr>
                  <td>{{App\Http\Controllers\UserController::getUserById($sent['author'])->username}} </td>
                  <td>{{$sent['comment']}}</td>
                  <td>{{$sent['rating']}}</td>
                </tr>
                @endforeach
